...
{
    auto progressCallback = [this](qreal progress) { m_state->setProgress(progress); };
    auto progressCallbackInv = [this](qreal progress) { m_state->setRegress(progress); };

    effects->registerTouchpadPinchShortcut(
        direction, 4, m_state->activateAction(), progressCallback);
    effects->registerTouchpadPinchShortcut(
        opposite(direction), 4, m_state->deactivateAction(), progressCallbackInv);
}
...
void TogglableGesture::addTouchscreenSwipeGesture(SwipeDirection direction, uint fingerCount)
{
    ...
    effects->registerTouchscreenSwipeShortcut(
        direction, 3, m_state->activateAction(), progressCallback);
    effects->registerTouchscreenSwipeShortcut(
        opposite(direction), 3, m_state->deactivateAction(), progressCallbackInv);
}
        ...
        effects->registerRealtimeTouchBorder(
            ...
            [this](ElectricBorder border,
                ...
                if (border == ElectricTop || border == ElectricBottom) {
                    progress = std::min(1.0, std::abs(deltaProgress.height()) / maxDelta);
                } else {
                    progress = std::min(1.0, std::abs(deltaProgress.width()) / maxDelta);
                }
