...
/**
 * The shortcut handler type provides a way to register global key shorcuts.
 *
 * Example usage:
 * @code
 * ShortcutHandler {
 ...
 * @endcode
 */
class KWIN_EXPORT shortcut_handler : public QObject, public QQmlParserStatus
{
    Q_OBJECT
     ...
     *
     * The shortcut name cannot be changed once the shortcut handler is constructed.
     */
     ...
     *
     * The key sequence cannot be changed once the shortcut handler is constructed.
     */
    Q_PROPERTY(QVariant sequence READ sequence WRITE setSequence NOTIFY sequenceChanged)

public:
    explicit shortcut_handler(QObject* parent = nullptr);
