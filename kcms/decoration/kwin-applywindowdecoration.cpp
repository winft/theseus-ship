...
int main(int argc, char** argv)
{
    ...
    parser->setApplicationDescription(
        i18n("This tool allows you to set the window decoration theme for the currently active "
             "session, without accidentally setting it to one that is either not available, or "
             "which is already set."));
    parser->addPositionalArgument(
        QStringLiteral("theme"),
        i18n("The name of the window decoration theme you wish to set for KWin. Passing a full "
             "path will attempt to find a theme in that directory, and then apply that if one can "
             "be deduced."));
    parser->addOption(QCommandLineOption(
        QStringLiteral("list-themes"),
    ...
    if (!parser->positionalArguments().isEmpty()) {
        ...
        if (requestedTheme.endsWith(QStringLiteral("/*"))) {
            ...
            // which has the main directory name and an asterix to say the cursors are all in that
            // directory, and since one of the main purposes of this tool is to allow adopting
            // things from a kns dialog, we handle that little weirdness here.
        ...
        if (requestedTheme.contains(QStringLiteral("/"))) {
            ...
            if (QFileInfo::exists(requestedTheme) && QFileInfo(requestedTheme).isDir()) {
                ...
                QStringList splitTheme
                    = requestedTheme.split(QStringLiteral("/"), Qt::SkipEmptyParts);
                if (splitTheme.count() > 3
                    && splitTheme[splitTheme.count() - 3] == QStringLiteral("aurorae")
                    && splitTheme[splitTheme.count() - 2] == QStringLiteral("themes")) {
                    // We think this is an aurorae theme, but let's just make a little more
                    // certain...
                    QString file(QStringLiteral("aurorae/themes/%1/metadata.desktop")
                                     .arg(splitTheme.last()));
                    QString path
                        = QStandardPaths::locate(QStandardPaths::GenericDataLocation, file);
                    if (!path.isEmpty()
                        && path == QStringLiteral("%1/metadata.desktop").arg(requestedTheme)) {
                        requestedTheme = QString("__aurorae__svg__").append(splitTheme.last());
                        themeResolved = true;
                        ts << i18n(
                                  "Resolved %1 to the KWin Aurorae theme \"%2\", and will attempt "
                                  "to set that as your current theme.")
                ...
                ts << i18n(
                    "You attempted to pass a file path, but this could not be resolved to a theme, "
                    "and we will have to abort, due to having no theme to set")
                   << Qt::endl;
                exitCode = -1;
            }
        }
        ...
        if (settings->theme() == requestedTheme) {
            ...
            if (index > -1) {
                ...
                if (settings->save()) {
                    ...
                    ts << i18n(
                        "Failed to save your theme settings - the reason is unknown, but this is "
                        "an unrecoverable error. You may find that simply trying again will work.");
                    exitCode = -1;
                }
            } else {
                ts << i18n(
        ...
        for (int i = 0; i < model->rowCount(); ++i) {
            ...
            if (settings->theme() == themeName) {
                ts << QStringLiteral(
                          " * %1 (theme name: %2 - current theme for this Plasma session)")
                          .arg(displayName)
                          .arg(themeName)
                   << Qt::endl;
            } else {
                ts << QStringLiteral(" * %1 (theme name: %2)").arg(displayName).arg(themeName)
                   << Qt::endl;
            }
        }
    } else {
