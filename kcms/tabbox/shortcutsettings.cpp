...
ShortcutItem::ShortcutItem(QAction* action, KActionCollection* actionCollection)
    ...
    , m_action(action)
{
    setGetDefaultImpl([this] { return m_actionCollection->defaultShortcut(m_action); });

    setIsDefaultImpl(
        [this] { return m_action->shortcut() == m_actionCollection->defaultShortcut(m_action); });

    setIsSaveNeededImpl([this] { return (m_action->shortcut() != m_savedShortcut); });
}
...
void ShortcutItem::writeConfig(KConfig* config)
{
    ...
    KGlobalAccel::self()->setShortcut(
        m_action, {m_action->shortcut()}, KGlobalAccel::NoAutoloading);
}
    ...
    , m_actionCollection(new KActionCollection(this, QStringLiteral("kwin")))
{
    ...
    auto addShortcut
        = [this](const KLocalizedString& name, const QKeySequence& sequence = QKeySequence()) {
    ...
    addShortcut(
        ki18nd("kwin", "Walk Through Windows of Current Application Alternative (Reverse)"));
}
