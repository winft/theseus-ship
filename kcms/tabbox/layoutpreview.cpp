    ...
    , m_item(nullptr)
{
    ...
    if (SwitcherItem* switcher = findSwitcher()) {
        m_item = switcher;
        static_cast<ExampleClientModel*>(switcher->model())
            ->showDesktopThumbnail(showDesktopThumbnail);
...
{
    const ThumbnailInfo desktopThumbnail = ThumbnailInfo{
        WindowThumbnailItem::Desktop, i18n("Show Desktop"), QStringLiteral("desktop")};
