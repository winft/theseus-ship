...
K_PLUGIN_FACTORY_WITH_JSON(KWinScreenEdgesConfigFactory,
                           "kcm_kwinscreenedges.json",
                           registerPlugin<theseus_ship::KWinScreenEdgesConfig>();
                           registerPlugin<theseus_ship::KWinScreenEdgeData>();)
    ...
    , m_data(new KWinScreenEdgeData(this))
{
    ...
    connect(m_form,
            &KWinScreenEdgesConfigForm::saveNeededChanged,
            this,
            &KWinScreenEdgesConfig::unmanagedWidgetChangeState);
    connect(m_form,
            &KWinScreenEdgesConfigForm::defaultChanged,
            this,
            &KWinScreenEdgesConfig::unmanagedWidgetDefaultState);
}
...
void KWinScreenEdgesConfig::load()
{
    ...
    m_form->setDefaultElectricBorderCornerRatio(
        m_data->settings()->defaultElectricBorderCornerRatioValue());
    m_form->reload();
}
...
static QList<KPluginMetaData> listBuiltinEffects()
{
    ...
    while (it.hasNext()) {
        it.next();
        if (const KPluginMetaData metaData = KPluginMetaData::fromJsonFile(it.filePath());
            metaData.isValid()) {
            ret.append(metaData);
        }
    }
...
{
    return KPackage::PackageLoader::self()->listPackages(QStringLiteral("KWin/Effect"),
...
void KWinScreenEdgesConfig::monitorInit()
{
    ...
    for (KPluginMetaData const& effect : effects) {
        ...
        if (!config.readEntry(effect.pluginId() + QStringLiteral("Enabled"),
                              effect.isEnabledByDefault())) {
            continue;
        }
        m_effects << effect.pluginId();
        m_form->monitorAddItem(effect.name());
        m_effectSettings[effect.pluginId()]
            = new KWinScreenEdgeEffectSettings(effect.pluginId(), this);
    }

    const QString scriptFolder = QStringLiteral("kwin/scripts/");
    const auto scripts = KPackage::PackageLoader::self()->listPackages(
        QStringLiteral("KWin/Script"), scriptFolder);
    ...
    for (const KPluginMetaData& script : scripts) {
        ...
        if (!config.readEntry(script.pluginId() + QStringLiteral("Enabled"),
                              script.isEnabledByDefault())) {
            continue;
        }
        m_scripts << script.pluginId();
        m_form->monitorAddItem(script.name());
        m_scriptSettings[script.pluginId()]
            = new KWinScreenEdgeScriptSettings(script.pluginId(), this);
    }
...
void KWinScreenEdgesConfig::monitorLoadSettings()
{
    ...
    m_form->monitorChangeEdge(
        como::win::electric_border::top,
        KWinScreenEdgesConfig::electricBorderActionFromString(m_data->settings()->top()));
    m_form->monitorChangeEdge(
        como::win::electric_border::top_right,
        KWinScreenEdgesConfig::electricBorderActionFromString(m_data->settings()->topRight()));
    m_form->monitorChangeEdge(
        como::win::electric_border::right,
        KWinScreenEdgesConfig::electricBorderActionFromString(m_data->settings()->right()));
    m_form->monitorChangeEdge(
        como::win::electric_border::bottom_right,
        KWinScreenEdgesConfig::electricBorderActionFromString(m_data->settings()->bottomRight()));
    m_form->monitorChangeEdge(
        como::win::electric_border::bottom,
        KWinScreenEdgesConfig::electricBorderActionFromString(m_data->settings()->bottom()));
    m_form->monitorChangeEdge(
        como::win::electric_border::bottom_left,
        KWinScreenEdgesConfig::electricBorderActionFromString(m_data->settings()->bottomLeft()));
    m_form->monitorChangeEdge(
        como::win::electric_border::left,
        KWinScreenEdgesConfig::electricBorderActionFromString(m_data->settings()->left()));
    m_form->monitorChangeEdge(
        como::win::electric_border::top_left,
        KWinScreenEdgesConfig::electricBorderActionFromString(m_data->settings()->topLeft()));
    ...
    m_form->monitorChangeEdge(m_data->settings()->borderActivatePresentWindows(),
...
{
    // Load ElectricBorderActions
    m_form->monitorChangeDefaultEdge(como::win::electric_border::top,
                                     KWinScreenEdgesConfig::electricBorderActionFromString(
                                         m_data->settings()->defaultTopValue()));
    m_form->monitorChangeDefaultEdge(como::win::electric_border::top_right,
                                     KWinScreenEdgesConfig::electricBorderActionFromString(
                                         m_data->settings()->defaultTopRightValue()));
    m_form->monitorChangeDefaultEdge(como::win::electric_border::right,
                                     KWinScreenEdgesConfig::electricBorderActionFromString(
                                         m_data->settings()->defaultRightValue()));
    m_form->monitorChangeDefaultEdge(como::win::electric_border::bottom_right,
                                     KWinScreenEdgesConfig::electricBorderActionFromString(
                                         m_data->settings()->defaultBottomRightValue()));
    m_form->monitorChangeDefaultEdge(como::win::electric_border::bottom,
                                     KWinScreenEdgesConfig::electricBorderActionFromString(
                                         m_data->settings()->defaultBottomValue()));
    m_form->monitorChangeDefaultEdge(como::win::electric_border::bottom_left,
                                     KWinScreenEdgesConfig::electricBorderActionFromString(
                                         m_data->settings()->defaultBottomLeftValue()));
    m_form->monitorChangeDefaultEdge(como::win::electric_border::left,
                                     KWinScreenEdgesConfig::electricBorderActionFromString(
                                         m_data->settings()->defaultLeftValue()));
    m_form->monitorChangeDefaultEdge(como::win::electric_border::top_left,
                                     KWinScreenEdgesConfig::electricBorderActionFromString(
                                         m_data->settings()->defaultTopLeftValue()));

    // Load effect-specific actions:

    // PresentWindows BorderActivateAll
    m_form->monitorChangeDefaultEdge(m_data->settings()->defaultBorderActivateAllValue(),
                                     PresentWindowsAll);

    // PresentWindows BorderActivate
    m_form->monitorChangeDefaultEdge(m_data->settings()->defaultBorderActivatePresentWindowsValue(),
                                     PresentWindowsCurrent);
    ...
    m_form->monitorChangeDefaultEdge(m_data->settings()->defaultBorderActivateTabBoxValue(),
                                     TabBox);
    // Alternative TabBox
    m_form->monitorChangeDefaultEdge(m_data->settings()->defaultBorderAlternativeActivateValue(),
                                     TabBoxAlternative);
}
...
void KWinScreenEdgesConfig::monitorSaveSettings()
{
    ...
    m_data->settings()->setTop(KWinScreenEdgesConfig::electricBorderActionToString(
        m_form->selectedEdgeItem(como::win::electric_border::top)));
    m_data->settings()->setTopRight(KWinScreenEdgesConfig::electricBorderActionToString(
        m_form->selectedEdgeItem(como::win::electric_border::top_right)));
    m_data->settings()->setRight(KWinScreenEdgesConfig::electricBorderActionToString(
        m_form->selectedEdgeItem(como::win::electric_border::right)));
    m_data->settings()->setBottomRight(KWinScreenEdgesConfig::electricBorderActionToString(
        m_form->selectedEdgeItem(como::win::electric_border::bottom_right)));
    m_data->settings()->setBottom(KWinScreenEdgesConfig::electricBorderActionToString(
        m_form->selectedEdgeItem(como::win::electric_border::bottom)));
    m_data->settings()->setBottomLeft(KWinScreenEdgesConfig::electricBorderActionToString(
        m_form->selectedEdgeItem(como::win::electric_border::bottom_left)));
    m_data->settings()->setLeft(KWinScreenEdgesConfig::electricBorderActionToString(
        m_form->selectedEdgeItem(como::win::electric_border::left)));
    m_data->settings()->setTopLeft(KWinScreenEdgesConfig::electricBorderActionToString(
        m_form->selectedEdgeItem(como::win::electric_border::top_left)));

    // Save effect-specific actions:

    // Present Windows
    m_data->settings()->setBorderActivateAll(
        m_form->monitorCheckEffectHasEdgeInt(PresentWindowsAll));
    m_data->settings()->setBorderActivatePresentWindows(
        m_form->monitorCheckEffectHasEdgeInt(PresentWindowsCurrent));
    ...
    m_data->settings()->setBorderAlternativeActivate(
        m_form->monitorCheckEffectHasEdgeInt(TabBoxAlternative));

    // Dinamically loaded effects
    int lastIndex = EffectCount;
    for (int i = 0; i < m_effects.size(); i++) {
        m_effectSettings[m_effects[i]]->setBorderActivate(
            m_form->monitorCheckEffectHasEdgeInt(lastIndex));
        ++lastIndex;
    }

    // Scripts
    for (int i = 0; i < m_scripts.size(); i++) {
        m_scriptSettings[m_scripts[i]]->setBorderActivate(
            m_form->monitorCheckEffectHasEdgeInt(lastIndex));
        ++lastIndex;
    }
}
...
void KWinScreenEdgesConfig::monitorShowEvent()
{
    ...
    m_form->monitorEnableEdge(como::win::electric_border::top,
                              !m_data->settings()->isTopImmutable());
    m_form->monitorEnableEdge(como::win::electric_border::top_right,
                              !m_data->settings()->isTopRightImmutable());
    m_form->monitorEnableEdge(como::win::electric_border::right,
                              !m_data->settings()->isRightImmutable());
    m_form->monitorEnableEdge(como::win::electric_border::bottom_right,
                              !m_data->settings()->isBottomRightImmutable());
    m_form->monitorEnableEdge(como::win::electric_border::bottom,
                              !m_data->settings()->isBottomImmutable());
    m_form->monitorEnableEdge(como::win::electric_border::bottom_left,
                              !m_data->settings()->isBottomLeftImmutable());
    m_form->monitorEnableEdge(como::win::electric_border::left,
                              !m_data->settings()->isLeftImmutable());
    m_form->monitorEnableEdge(como::win::electric_border::top_left,
                              !m_data->settings()->isTopLeftImmutable());

    // Disable ElectricBorderCornerRatio if entry is immutable
    m_form->setElectricBorderCornerRatioEnabled(
        !m_data->settings()->isElectricBorderCornerRatioImmutable());
}
