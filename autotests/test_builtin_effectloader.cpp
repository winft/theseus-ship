...
void TestBuiltInEffectLoader::testKnownEffects()
{
    ...
    expectedEffects << QStringLiteral("blur") << QStringLiteral("colorpicker")
                    << QStringLiteral("contrast") << QStringLiteral("coverswitch")
                    << QStringLiteral("cube") << QStringLiteral("cubeslide")
                    << QStringLiteral("desktopgrid") << QStringLiteral("diminactive")
                    << QStringLiteral("fallapart") << QStringLiteral("flipswitch")
                    << QStringLiteral("glide") << QStringLiteral("highlightwindow")
                    << QStringLiteral("invert") << QStringLiteral("kscreen")
                    << QStringLiteral("lookingglass") << QStringLiteral("magiclamp")
                    << QStringLiteral("magnifier") << QStringLiteral("mouseclick")
                    << QStringLiteral("mousemark") << QStringLiteral("presentwindows")
                    << QStringLiteral("resize") << QStringLiteral("screenedge")
                    << QStringLiteral("screenshot") << QStringLiteral("sheet")
                    << QStringLiteral("showfps") << QStringLiteral("showpaint")
                    << QStringLiteral("slide") << QStringLiteral("slideback")
                    << QStringLiteral("slidingpopups") << QStringLiteral("snaphelper")
                    << QStringLiteral("startupfeedback") << QStringLiteral("thumbnailaside")
                    << QStringLiteral("touchpoints") << QStringLiteral("trackmouse")
...
void TestBuiltInEffectLoader::testSupported_data()
{
    ...
    QTest::newRow("CoverSwitch-GL-no-anim")
        << QStringLiteral("coverswitch") << false << oc << false;
    ...
    QTest::newRow("WobblyWindows-GL-no-anim")
        << QStringLiteral("wobblywindows") << false << oc << false;
    ...
    QTest::newRow("Fade - Scripted + kwin4_effect")
        << QStringLiteral("kwin4_effect_fade") << false << xc << true;
}
...
void TestBuiltInEffectLoader::testLoadEffect_data()
{
    ...
    //     QTest::newRow("CoverSwitch-GL")                 << QStringLiteral("coverswitch")       <<
    //     true  << oc;
    QTest::newRow("Cube") << QStringLiteral("cube") << false << xc;
    // TODO: needs GL mocking
    //     QTest::newRow("Cube-GL")                        << QStringLiteral("cube")              <<
    //     true  << oc;
    ...
    //     QTest::newRow("ShowFps")                        << QStringLiteral("showfps")           <<
    //     true  << xc;
    ...
    //     QTest::newRow("StartupFeedback-GL")             << QStringLiteral("startupfeedback")   <<
    //     true  << oc;
    ...
    // TODO: Accesses EffectFrame and crashes
    //     QTest::newRow("WindowGeometry")                 << QStringLiteral("windowgeometry")    <<
    //     true  << xc;
    ...
    QTest::newRow("Fade - Scripted + kwin4_effect")
        << QStringLiteral("kwin4_effect_fade") << false << xc;
}
...
void TestBuiltInEffectLoader::testLoadEffect()
{
    ...
    // now if we wait for the events being processed, the effect will get deleted and it should load
    // again
    QTest::qWait(1);
    QCOMPARE(loader.loadEffect(name), expected);
    // signal spy should have got the signal if it was expected
...
void TestBuiltInEffectLoader::testLoadBuiltInEffect_data()
{
    ...
    QTest::newRow("HighlightWindow")
        << KWin::BuiltInEffect::HighlightWindow << QStringLiteral("highlightwindow") << true << xc
        << checkDefault;
    // supported but not enabled by default
    QTest::newRow("LookingGlass-GL")
        << KWin::BuiltInEffect::LookingGlass << QStringLiteral("lookingglass") << true << oc
        << checkDefault;
    // not enabled by default
    QTest::newRow("MouseClick") << KWin::BuiltInEffect::MouseClick << QStringLiteral("mouseclick")
                                << true << xc << checkDefault;
    // Force an Effect which will load
    QTest::newRow("MouseClick-Force") << KWin::BuiltInEffect::MouseClick
                                      << QStringLiteral("mouseclick") << true << xc << forceFlags;
    // Force an Effect which is not supported
    QTest::newRow("LookingGlass-Force")
        << KWin::BuiltInEffect::LookingGlass << QStringLiteral("lookingglass") << false << xc
        << forceFlags;
    // Force the Effect as supported
    QTest::newRow("LookingGlass-Force-GL")
        << KWin::BuiltInEffect::LookingGlass << QStringLiteral("lookingglass") << true << oc
        << forceFlags;
    // Enforce no load of effect which is enabled by default
    QTest::newRow("HighlightWindow-DontLoad")
        << KWin::BuiltInEffect::HighlightWindow << QStringLiteral("highlightwindow") << false << xc
        << dontLoadFlags;
    // Enforce no load of effect which is not enabled by default, but enforced
    QTest::newRow("MouseClick-DontLoad")
        << KWin::BuiltInEffect::MouseClick << QStringLiteral("mouseclick") << false << xc
        << dontLoadFlags;
}
