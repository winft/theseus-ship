...
void TestXdgShellClient::testTransientPositionAfterRemap()
{
    ...
    auto transient = Test::render_and_wait_for_shown(transientSurface, positioner.initialSize(), Qt::blue);
    QVERIFY(transient);
...
void TestXdgShellClient::testWindowOpensLargerThanScreen()
{
    ...
    auto c = Test::render_and_wait_for_shown(surface, screens()->size(0), Qt::blue);
    QVERIFY(c);
    QVERIFY(c->control->active());
    QCOMPARE(win::frame_to_client_size(c, c->size()), screens()->size(0));
    QVERIFY(win::decoration(c));
...
void TestXdgShellClient::testAppMenu()
{
    ...
    QCOMPARE(c->control->application_menu(), std::make_tuple(QString("service.name"), QString("object/path")));
