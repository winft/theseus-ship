...
void DecorationInputTest::testPressToMove()
{
    ...
    quint32 timestamp = 1;
    MOTION(QPoint(c->frameGeometry().center().x(), c->pos().y() + c->clientPos().y() / 2));
    QCOMPARE(c->control()->move_resize().cursor, CursorShape(Qt::ArrowCursor));

    PRESS;
    QVERIFY(!win::is_move(c));
    QFETCH(QPoint, offset);
    MOTION(QPoint(c->frameGeometry().center().x(), c->pos().y() + c->clientPos().y() / 2) + offset);
    ...
    MOTION(QPoint(c->frameGeometry().center().x(), c->pos().y() + c->clientPos().y() / 2) + offset2);
    QVERIFY(win::is_move(c));
    QCOMPARE(startMoveResizedSpy.count(), 2);
    QFETCH(QPoint, offset3);
    MOTION(QPoint(c->frameGeometry().center().x(), c->pos().y() + c->clientPos().y() / 2) + offset3);
...
void DecorationInputTest::testTapToMove()
{
    ...
    quint32 timestamp = 1;
    QPoint p = QPoint(c->frameGeometry().center().x(), c->pos().y() + c->clientPos().y() / 2);
    ...
    QVERIFY(!win::is_move(c));
    QFETCH(QPoint, offset2);
    kwinApp()->platform()->touchMotion(1, QPoint(c->frameGeometry().center().x(), c->pos().y() + c->clientPos().y() / 2) + offset2, timestamp++);
    QVERIFY(win::is_move(c));
    QCOMPARE(startMoveResizedSpy.count(), 2);
    QFETCH(QPoint, offset3);
    kwinApp()->platform()->touchMotion(1, QPoint(c->frameGeometry().center().x(), c->pos().y() + c->clientPos().y() / 2) + offset3, timestamp++);
...
void DecorationInputTest::testModifierClickUnrestrictedMove()
{
    ...
    Cursor::setPos(QPoint(c->frameGeometry().center().x(), c->pos().y() + c->clientPos().y() / 2));
...
void DecorationInputTest::testTouchEvents()
{
    ...
    kwinApp()->platform()->touchDown(0, tapPoint, timestamp++);
    QVERIFY(input()->touch()->decoration());
    QCOMPARE(input()->touch()->decoration()->decoration(), win::decoration(c));
