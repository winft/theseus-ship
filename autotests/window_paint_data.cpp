...
TEST_CASE("window paint data", "[legacy],[effect]")
{
    MockEffectWindow w;
    ...
    SECTION("operator multiply assign")
    {
        WindowPaintData data(&w);
    ...
    SECTION("multiply opacity")
    {
        WindowPaintData data(&w);
        QCOMPARE(0.2, data.multiplyOpacity(0.2));
        QCOMPARE(0.2, data.opacity());
        REQUIRE_THAT(data.multiplyOpacity(3.0), Catch::Matchers::WithinAbs(0.6, 0.0001));
        REQUIRE_THAT(data.opacity(), Catch::Matchers::WithinAbs(0.6, 0.0001));
        // just for safety
        QCOMPARE(1.0, data.brightness());
        QCOMPARE(1.0, data.saturation());
    }

    SECTION("multiply saturation")
    {
        WindowPaintData data(&w);
        QCOMPARE(0.2, data.multiplySaturation(0.2));
        QCOMPARE(0.2, data.saturation());
        REQUIRE_THAT(data.multiplySaturation(3.0), Catch::Matchers::WithinAbs(0.6, 0.0001));
        REQUIRE_THAT(data.saturation(), Catch::Matchers::WithinAbs(0.6, 0.0001));
        // just for safety
        QCOMPARE(1.0, data.brightness());
        QCOMPARE(1.0, data.opacity());
    }

    SECTION("multiply brightness")
    {
        WindowPaintData data(&w);
        QCOMPARE(0.2, data.multiplyBrightness(0.2));
        QCOMPARE(0.2, data.brightness());
        REQUIRE_THAT(data.multiplyBrightness(3.0), Catch::Matchers::WithinAbs(0.6, 0.0001));
        REQUIRE_THAT(data.brightness(), Catch::Matchers::WithinAbs(0.6, 0.0001));
        // just for safety
        QCOMPARE(1.0, data.opacity());
        QCOMPARE(1.0, data.saturation());
    }
