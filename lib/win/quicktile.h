...
void check_quicktile_maximization_zones(Win* win, int xroot, int yroot)
{
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
        ...
        auto area = space_window_area(
            win->space, area_option::maximize, QPoint(xroot, yroot), get_subspace(*win));
        ...
        if (mode != quicktiles::none) {
            auto ratio = win->space.options->qobject->electricBorderCornerRatio();
            if (yroot <= area.y() + area.height() * ratio) {
                mode |= quicktiles::top;
            } else if (yroot >= area.y() + area.height() - area.height() * ratio) {
                mode |= quicktiles::bottom;
            }
        } else if (win->space.options->qobject->electricBorderMaximize() && yroot <= area.y() + 5
                   && win->isMaximizable()) {
...
void set_quicktile_mode(Win* win, quicktiles mode, bool keyboard)
{
    ...
    if (win->geo.update.max_mode != maximize_mode::restore) {
        ...
        if (mode != quicktiles::none) {
            ...
            win->setFrameGeometry(
                electric_border_maximize_geometry(win, ref_pos, get_subspace(*win)));
    ...
    if (mode != quicktiles::none) {
        ...
        if (mode != quicktiles::none) {
            ...
            win->setFrameGeometry(
                electric_border_maximize_geometry(win, target_screen, get_subspace(*win)));
        }
