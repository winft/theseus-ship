...
void SwipeGestureHandler::componentComplete()
{
    ...
    std::function<void(EffectsHandler*,
                       SwipeDirection dir,
                       uint fingerCount,
                       QAction * onUp,
    ...
    registrator(
        effects, SwipeDirection(m_direction), m_fingerCount, m_action, [this](qreal progress) {
...
void PinchGestureHandler::componentComplete()
{
    ...
    registrator(
        effects, PinchDirection(m_direction), m_fingerCount, m_action, [this](qreal progress) {
            setProgress(progress);
        });
}
