...
ScriptedQuickSceneEffect::ScriptedQuickSceneEffect()
{
    ...
    connect(&m_visibleTimer, &QTimer::timeout, this, [this]() { setRunning(false); });
}
...
void ScriptedQuickSceneEffect::setMetaData(const KPluginMetaData& metaData,
                                           KSharedConfigPtr const& config)
{
    m_requestedEffectChainPosition = metaData.value(QStringLiteral("X-KDE-Ordering"), 50);

    auto cg = config->group(QStringLiteral("Effect-%1").arg(metaData.pluginId()));
    const QString configFilePath
        = QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                                 QLatin1String("kwin/effects/") + metaData.pluginId()
    ...
    connect(m_configLoader,
            &KConfigLoader::configChanged,
            this,
            &ScriptedQuickSceneEffect::configurationChanged);
}
