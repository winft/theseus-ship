...
void ZoomEffect::paintScreen(int mask, const QRegion& region, ScreenPaintData& data)
{
    ...
    GLRenderTarget::pushRenderTarget(offscreenData->framebuffer.data());
    ...
    if (zoom != 1.0) {
        ...
        if (mousePointer != MousePointerHide) {
            // Draw the mouse-texture at the position matching to zoomed-in image of the desktop.
            // Hiding the previous mouse-cursor and drawing our own fake mouse-cursor is needed to
            // be able to scale the mouse-cursor up and to re-position those mouse-cursor to match
            // to the chosen zoom-level.
