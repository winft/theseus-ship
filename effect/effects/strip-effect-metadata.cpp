...
static bool stripMetaData(const QString& fileName, const QString& target)
{
    ...
    if (!originalFile.open(QFile::ReadOnly)) {
        qWarning(
            "Failed to open %s: %s", qPrintable(fileName), qPrintable(originalFile.errorString()));
        return false;
    }
...
int main(int argc, char** argv)
{
    ...
    QCommandLineOption inputOption(
        QStringLiteral("source"), QStringLiteral("input file"), QStringLiteral("path/to/file"));
    parser.addOption(inputOption);

    QCommandLineOption outputOption(
        QStringLiteral("output"), QStringLiteral("output file"), QStringLiteral("path/to/file"));
