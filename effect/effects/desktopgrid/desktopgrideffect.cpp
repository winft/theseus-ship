    ...
    , m_realtimeToggleAction(new QAction(this))
{
    ...
    KGlobalAccel::self()->setDefaultShortcut(m_toggleAction,
                                             QList<QKeySequence>() << (Qt::META | Qt::Key_F8));
    KGlobalAccel::self()->setShortcut(m_toggleAction,
                                      QList<QKeySequence>() << (Qt::META | Qt::Key_F8));
    ...
    for (const int& border : touchActivateBorders) {
        ...
        effects->registerRealtimeTouchBorder(
            ...
            [this](ElectricBorder border, const QSizeF& deltaProgress, const EffectScreen* screen) {
                ...
                const bool wasInProgress = m_partialActivationFactor > 0;
                int maxDelta = 500; // Arbitrary logical pixels value seems to behave better than
                                    // scaledScreenSize
                if (border == ElectricTop || border == ElectricBottom) {
                    maxDelta = (screen->geometry().height() / rows)
                        * (rows - (effects->currentDesktop() % rows));
                    m_partialActivationFactor
                        = std::min(1.0, qAbs(deltaProgress.height()) / maxDelta);
                } else {
                    maxDelta = (screen->geometry().width() / columns)
                        * (columns - (effects->currentDesktop() % columns));
                    m_partialActivationFactor
                        = std::min(1.0, qAbs(deltaProgress.width()) / maxDelta);
                }
