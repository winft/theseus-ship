    ...
    , m_captionFrame(nullptr)
{
    ...
    connect(
        flipSwitchCurrentAction, &QAction::triggered, this, &FlipSwitchEffect::toggleActiveCurrent);
    ...
    connect(
        flipSwitchAllAction, &QAction::triggered, this, &FlipSwitchEffect::toggleActiveAllDesktops);
    connect(KGlobalAccel::self(),
            &KGlobalAccel::globalShortcutChanged,
            this,
            &FlipSwitchEffect::globalShortcutChanged);
...
void FlipSwitchEffect::prePaintScreen(ScreenPrePaintData& data,
...
void FlipSwitchEffect::paintScreen(int mask, const QRegion& region, ScreenPaintData& data)
{
    ...
    if (m_active) {
        ...
        int tabIndex = index;
        if (m_mode == TabboxMode) {
            for (auto const& direction : qAsConst(m_scheduledDirections)) {
            ...
            if (!m_scheduledDirections.isEmpty()
                && m_scheduledDirections.head() == DirectionBackward) {
                index--;
                if (index < 0)
                    index = tempList.count() + index;
                w = tempList.at(index);
        ...
        if (m_animation && !m_scheduledDirections.isEmpty()) {
            ...
            if (ItemInfo* info = m_windows.value(w, 0)) {
                ...
                data.translate(-(w->x() - m_screenArea.x() + data.xTranslation())
                               * m_startStopTimeLine.currentValue());

                data.translate(m_screenArea.width() * m_xPosition
                                   * m_startStopTimeLine.currentValue(),
                               (m_screenArea.y() + m_screenArea.height() * m_yPosition
                                - (w->y() + w->height() + data.yTranslation()))
                                   * m_startStopTimeLine.currentValue());
                data.translate(-(m_screenArea.width() * 0.25f) * distance
                                   * m_startStopTimeLine.currentValue(),
                               -(m_screenArea.height() * 0.10f) * distance
                                   * m_startStopTimeLine.currentValue(),
        ...
        for (auto const& w : qAsConst(m_flipOrderedWindows)) {
            ...
            data.translate(
                -(m_screenArea.width() * 0.25f) * distance * m_startStopTimeLine.currentValue(),
...
void FlipSwitchEffect::prePaintWindow(EffectWindow* w,
...
void FlipSwitchEffect::setActive(bool activate, FlipSwitchMode mode)
{
    if (activate) {
        ...
        if (effects->numScreens() > 1) {
            ...
            QRect fullRect
                = effects->clientArea(FullArea, m_activeScreen, effects->currentDesktop());
            float fovy = 60.0f;
            float aspect = 1.0f;
            float zNear = 0.1f;
            float zFar = 100.0f;
...
void FlipSwitchEffect::grabbedKeyboardEvent(QKeyEvent* e)
{
    if (e->type() == QEvent::KeyPress) {
        ...
        case Qt::Key_Tab: {
            ...
            if (!found) {
                for (int i = effects->stackingOrder().count() - 1;
                     i > effects->stackingOrder().indexOf(m_selectedWindow);
                     i--) {
                    if (isSelectableWindow(effects->stackingOrder().at(i))) {
                        m_selectedWindow = effects->stackingOrder().at(i);
                        found = true;
        ...
        case Qt::Key_Backtab: {
            ...
            if (m_windows.isEmpty())
                return; // sanity check
            bool found = false;
            for (int i = effects->stackingOrder().indexOf(m_selectedWindow) + 1;
                 i < effects->stackingOrder().count();
                 i++) {
                if (isSelectableWindow(effects->stackingOrder().at(i))) {
                    m_selectedWindow = effects->stackingOrder().at(i);
                    found = true;
...
void FlipSwitchEffect::updateCaption()
{
    ...
    if (m_selectedWindow->isDesktop()) {
        m_captionFrame->setText(
            i18nc("Special entry in alt+tab list for minimizing all windows", "Show Desktop"));
        static QPixmap pix
            = QIcon::fromTheme(QStringLiteral("user-desktop")).pixmap(m_captionFrame->iconSize());
