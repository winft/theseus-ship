...
const static QString s_errorAlreadyTaking
    = QStringLiteral("org.kde.kwin.Screenshot.Error.AlreadyTaking");
const static QString s_errorAlreadyTakingMsg = QStringLiteral("A screenshot is already been taken");
const static QString s_errorNotAuthorized
    = QStringLiteral("org.kde.kwin.Screenshot.Error.NoAuthorized");
const static QString s_errorNotAuthorizedMsg
...
const static QString s_errorInvalidArea
    = QStringLiteral("org.kde.kwin.Screenshot.Error.InvalidArea");
const static QString s_errorInvalidAreaMsg = QStringLiteral("Invalid area requested");
const static QString s_errorInvalidScreen
    = QStringLiteral("org.kde.kwin.Screenshot.Error.InvalidScreen");
...
const static QString s_errorScreenMissing
    = QStringLiteral("org.kde.kwin.Screenshot.Error.ScreenMissing");
...
public:
    ScreenShotSinkPipe1(ScreenShotDBusInterface1* interface,
                        int fileDescriptor,
                        QDBusMessage replyMessage = QDBusMessage());
    ...
    : m_sources(sources)
{
    for (ScreenShotSource1* source : sources) {
        ...
        connect(source,
                &ScreenShotSource1::cancelled,
                this,
                &ScreenShotSourceMulti1::handleSourceCancelled);
        connect(source,
                &ScreenShotSource1::completed,
                this,
                &ScreenShotSourceMulti1::handleSourceCompleted);
    }
}
...
void ScreenShotSink1::cancel()
{
    if (m_replyMessage.isDelayedReply()) {
        QDBusConnection::sessionBus().send(
            m_replyMessage.createErrorReply(s_errorCancelled, s_errorCancelledMsg));
    }
}

ScreenShotSinkPipe1::ScreenShotSinkPipe1(ScreenShotDBusInterface1* interface,
                                         int fileDescriptor,
...
void ScreenShotSinkPipe1::flushMulti(const QList<QImage>& images)
{
    ...
    QtConcurrent::run(
        [](int fd, const QList<QImage>& images) {
            QFile file;
            if (file.open(fd, QIODevice::WriteOnly, QFileDevice::AutoCloseHandle)) {
                QDataStream ds(&file);
...
static xcb_pixmap_t xpixmapFromImage(const QImage& image)
{
    ...
    xcb_create_pixmap(
        c, image.depth(), pixmap, effects->x11RootWindow(), image.width(), image.height());
...
static QString saveTempImage(const QImage& image)
{
    ...
    QTemporaryFile temp(QDir::tempPath() + QDir::separator()
                        + QLatin1String("kwin_screenshot_XXXXXX.png"));
    ...
    KNotification::event(
        ...
        i18nc(
            "Notification with path to screenshot file", "Screenshot saved to %1", temp.fileName()),
    ...
    , m_effect(effect)
{
    QDBusConnection::sessionBus().registerObject(
        QStringLiteral("/Screenshot"), this, QDBusConnection::ExportScriptableContents);
}
    ...
    if (reply.isValid()) {
        ...
        if (!interfaces.contains(s_dbusInterfaceName)) {
            ...
            qCWarning(KWIN_SCREENSHOT)
                << "Process" << pid
                << "tried to take a screenshot without being granted to DBus interface"
                << s_dbusInterfaceName;
            return false;
        }
    } else {
...
void ScreenShotDBusInterface1::screenshotWindowUnderCursor(int mask)
{
    ...
    while (it != first) {
        hoveredWindow = *(--it);
        if (hoveredWindow->isOnCurrentDesktop() && !hoveredWindow->isMinimized()
            && !hoveredWindow->isDeleted() && hoveredWindow->frameGeometry().contains(cursor)) {
            break;
        }
    ...
    if (hoveredWindow) {
        takeScreenShot(
            hoveredWindow, ScreenShotFlags(mask), new ScreenShotSinkXpixmap1(this, message()));
    }
}
...
QString ScreenShotDBusInterface1::interactive(int mask)
{
    ...
    effects->startInteractiveWindowSelection([this, mask, replyMessage](EffectWindow* window) {
        ...
        if (!window) {
            QDBusConnection::sessionBus().send(
                replyMessage.createErrorReply(s_errorCancelled, s_errorCancelledMsg));
            return;
        }
...
void ScreenShotDBusInterface1::interactive(QDBusUnixFileDescriptor fd, int mask)
{
    ...
    effects->startInteractiveWindowSelection([this, fileDescriptor, mask](EffectWindow* window) {
        ...
        takeScreenShot(
            window, ScreenShotFlags(mask), new ScreenShotSinkPipe1(this, fileDescriptor));
    });
...
QString ScreenShotDBusInterface1::screenshotFullscreen(bool captureCursor)
{
    ...
    takeScreenShot(
        effects->virtualScreenGeometry(), flags, new ScreenShotSinkFile1(this, message()));
                                                    ...
                                                    bool shouldReturnNativeSize)
{
    ...
    takeScreenShot(
        effects->virtualScreenGeometry(), flags, new ScreenShotSinkPipe1(this, fileDescriptor));
}
...
QString
ScreenShotDBusInterface1::screenshotArea(int x, int y, int width, int height, bool captureCursor)
{
    if (!checkCall()) {
        return QString();
    }
...
void ScreenShotDBusInterface1::bind(ScreenShotSink1* sink, ScreenShotSource1* source)
{
    ...
    connect(m_source.data(),
            &ScreenShotSource1::cancelled,
            this,
            &ScreenShotDBusInterface1::handleSourceCancelled);
    connect(m_source.data(),
            &ScreenShotSource1::completed,
            this,
            &ScreenShotDBusInterface1::handleSourceCompleted);
}

void ScreenShotDBusInterface1::takeScreenShot(EffectScreen* screen,
                                              ScreenShotFlags flags,
...
void ScreenShotDBusInterface1::takeScreenShot(const QRect& area,
                                              ScreenShotFlags flags,
                                              ScreenShotSink1* sink)
{
...
void ScreenShotDBusInterface1::takeScreenShot(EffectWindow* window,
                                              ScreenShotFlags flags,
                                              ScreenShotSink1* sink)
{
