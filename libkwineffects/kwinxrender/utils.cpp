...
XRenderPicture xRenderFill(const xcb_render_color_t& c)
{
    ...
    xcb_render_fill_rectangles(
        XRenderUtils::s_connection, XCB_RENDER_PICT_OP_SRC, fill, c, 1, &rect);
    return fill;
}
...
static xcb_render_picture_t createPicture(xcb_pixmap_t pix, int depth)
{
    ...
    if (!s_renderFormats.contains(depth)) {
        xcb_render_query_pict_formats_reply_t* formats = xcb_render_query_pict_formats_reply(
            c, xcb_render_query_pict_formats_unchecked(c), nullptr);
        if (!formats) {
            return XCB_RENDER_PICTURE_NONE;
        }
...
XFixesRegion::XFixesRegion(const QRegion& region)
{
    ...
    xcb_xfixes_create_region(
        XRenderUtils::s_connection, m_region, xrects.count(), xrects.constData());
}
...
void setXRenderOffscreen(bool b)
{
    ...
    if (s_renderOffscreen < 0) {
        s_renderOffscreen = 0;
        qCWarning(LIBKWINXRENDERUTILS)
            << "*** SOMETHING IS MESSED UP WITH YOUR setXRenderOffscreen() USAGE ***";
    }
}
...
void xRenderPopTarget()
{
    ...
    if (s_renderOffscreen < 0) {
        s_renderOffscreen = 0;
        qCWarning(LIBKWINXRENDERUTILS)
            << "*** SOMETHING IS MESSED UP WITH YOUR xRenderPopTarget() USAGE ***";
    }
}
...
struct PictFormatData {
    PictFormatData()
    {
        ...
        reply = xcb_render_query_pict_formats_reply(
            s_connection, xcb_render_query_pict_formats_unchecked(s_connection), nullptr);
